...
import { getChats, deleteChatById } from 'apiSdk/chats';
import { ChatInterface } from 'interfaces/chat';

type ColumnType = ColumnDef<ChatInterface, unknown>;
...
export function ChatListPage(props: ChatListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getChats({
        relations: ['user', 'organization', 'message.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ChatInterface>>(
    () => `/chats?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ChatInterface) => {
    if (hasAccess('chat', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/chats/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/chats/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
              ...
              {hasAccess('chat', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/chats/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('chat', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="chat" />
        </Flex>

        {hasAccess('chat', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/chats/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
